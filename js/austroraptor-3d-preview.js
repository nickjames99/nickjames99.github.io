...
if (stage && dinosaurSelect) {
  ...
  canvasHost.id = "austro3dPreviewHost";
  ...
  canvas.id = "austro3dPreview";
  canvas.setAttribute("aria-label", "Interactive 3D Austroraptor preview");
  ...
  status.className = "austro-3d-status";
  status.textContent = "Loading Austroraptor 3D…";
  ...
  async function loadMasks() {
    ...
    ["a", "b", "c", "d"].forEach(pattern => {
      ["male", "female"].forEach(sex => {
        [1, 2].forEach(set => {
          ...
          const source =
            `./assets/textures/austroraptor/${pattern}-${sex}-channels-${set}.png`;
          tasks.push(
    ...
    if (failedMasks.length) {
      console.warn(
        `Austroraptor 3D preview continued with ${failedMasks.length} missing mask texture(s).`
      );
    }
  }
  ...
  function installBodyShader(material) {
    ...
    material.customProgramCacheKey = () => "austroraptor-original-layer-masks-v1";
    material.color.set(0xffffff);
    material.needsUpdate = true;
  }

  function isAllosaurus() {
    return dinosaurSelect.value === "austro";
  }

  function showThreeDimensionalError() {
    status.textContent = "Austroraptor 3D preview could not be loaded";
    updateVisibility();
  }
  ...
  try {
    ...
    loader.load(
      "./assets/models/austroraptor.glb",
      async gltf => {
        await masksReady;
      ...
      progress => {
        ...
        status.textContent = `Loading Austroraptor 3D · ${percent}%`;
      },
      error => {
        console.error("Austroraptor 3D preview failed:", error);
        failed = true;
