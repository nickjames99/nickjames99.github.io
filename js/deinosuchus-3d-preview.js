...
if (stage && dinosaurSelect) {
  // Deinosuchus is 3D-only. The shadow root prevents legacy preview code
  // from finding or restyling its WebGL canvas.
  document.getElementById("deinoPreview")?.remove();
  ...
  canvasHost.id = "deino3dPreviewHost";
  ...
  canvas.id = "deino3dPreview";
  canvas.setAttribute("aria-label", "Interactive 3D Deinosuchus preview");
  ...
  status.className = "deino-3d-status";
  status.textContent = "Loading Deinosuchus 3D…";
  ...
  async function loadMasks() {
    ...
    ["a", "b", "c"].forEach(pattern => {
      ["male", "female"].forEach(sex => {
        [1, 2].forEach(set => {
          ...
          const source =
            `./assets/textures/deinosuchus/${pattern}-${sex}-channels-${set}.png`;
          tasks.push(
    ...
    if (failedMasks.length) {
      console.warn(
        `Deinosuchus 3D preview continued with ${failedMasks.length} missing mask texture(s).`
      );
    }
  }
  ...
  function installBodyShader(material) {
    ...
    material.customProgramCacheKey = () => "deinosuchus-original-layer-masks-v1";
    material.color.set(0xffffff);
    material.needsUpdate = true;
  }

  function isAllosaurus() {
    return dinosaurSelect.value === "deino";
  }

  function showThreeDimensionalError() {
    status.textContent = "Deinosuchus 3D preview could not be loaded";
    updateVisibility();
  }
  ...
  try {
    ...
    loader.load(
      ...
      progress => {
        ...
        status.textContent = `Loading Deinosuchus 3D · ${percent}%`;
      },
      error => {
        console.error("Deinosuchus 3D preview failed:", error);
        failed = true;
