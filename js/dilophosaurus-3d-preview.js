...
if (stage && dinosaurSelect) {
  // Dilophosaurus is 3D-only. The shadow root prevents legacy preview code
  // from finding or restyling its WebGL canvas.
  document.getElementById("diloPreview")?.remove();
  ...
  canvasHost.id = "dilo3dPreviewHost";
  ...
  canvas.id = "dilo3dPreview";
  canvas.setAttribute("aria-label", "Interactive 3D Dilophosaurus preview");
  ...
  status.className = "dilo-3d-status";
  status.textContent = "Loading Dilophosaurus 3D…";
  ...
  async function loadMasks() {
    ...
    ["a", "b", "c"].forEach(pattern => {
      ["male", "female"].forEach(sex => {
        [1, 2].forEach(set => {
          ...
          const source =
            `./assets/textures/dilophosaurus/${pattern}-${sex}-channels-${set}.png`;
          tasks.push(
    ...
    if (failedMasks.length) {
      console.warn(
        `Dilophosaurus 3D preview continued with ${failedMasks.length} missing mask texture(s).`
      );
    }
  }
  ...
  function installBodyShader(material) {
    ...
    material.customProgramCacheKey = () => "dilophosaurus-original-layer-masks-v1";
    material.color.set(0xffffff);
    material.needsUpdate = true;
  }

  function isAllosaurus() {
    return dinosaurSelect.value === "dilo";
  }

  function showThreeDimensionalError() {
    status.textContent = "Dilophosaurus 3D preview could not be loaded";
    updateVisibility();
  }
  ...
  try {
    ...
    loader.load(
      ...
      async gltf => {
        ...
        gltf.scene.traverse(item => {
          ...
          materials.filter(Boolean).forEach(material => {
            ...
            if (/(body|quills)/i.test(material.name)) installBodyShader(material);
      ...
      progress => {
        ...
        status.textContent = `Loading Dilophosaurus 3D · ${percent}%`;
      },
      error => {
        console.error("Dilophosaurus 3D preview failed:", error);
        failed = true;
