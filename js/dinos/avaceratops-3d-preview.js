...
if (stage && dinosaurSelect) {
  // Avaceratops is 3D-only. The shadow root prevents legacy preview code
  // from finding or restyling its WebGL canvas.
  document.getElementById("avaPreview")?.remove();
  ...
  canvasHost.id = "ava3dPreviewHost";
  ...
  canvas.id = "ava3dPreview";
  canvas.setAttribute("aria-label", "Interactive 3D Avaceratops preview");
  ...
  status.className = "ava-3d-status";
  status.textContent = "Loading Avaceratops 3D…";
  ...
  async function loadMasks() {
    ...
    ["a"].forEach(pattern => {
      ["male", "female"].forEach(sex => {
        [1, 2].forEach(set => {
          ...
          const source =
            `./assets/textures/avaceratops/${pattern}-${sex}-channels-${set}.png`;
          tasks.push(
    ...
    if (failedMasks.length) {
      console.warn(
        `Avaceratops 3D preview continued with ${failedMasks.length} missing mask texture(s).`
      );
    }
  }
  ...
  function installBodyShader(material) {
    ...
    material.customProgramCacheKey = () => "avaceratops-original-layer-masks-v1";
    material.color.set(0xffffff);
    material.needsUpdate = true;
  }

  function isAllosaurus() {
    return dinosaurSelect.value === "ava";
  }

  function showThreeDimensionalError() {
    status.textContent = "Avaceratops 3D preview could not be loaded";
    updateVisibility();
  }
  ...
  try {
    ...
    loader.load(
      ...
      progress => {
        ...
        status.textContent = `Loading Avaceratops 3D · ${percent}%`;
      },
      error => {
        console.error("Avaceratops 3D preview failed:", error);
        failed = true;
