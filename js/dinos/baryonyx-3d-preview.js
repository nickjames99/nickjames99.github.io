...
if (stage && dinosaurSelect) {
  document.getElementById("bary3dPreviewHost")?.remove();
  // Baryonyx is 3D-only. The shadow root prevents legacy preview code
  // from finding or restyling its WebGL canvas.
  document.getElementById("baryPreview")?.remove();
  ...
  canvasHost.id = "bary3dPreviewHost";
  ...
  canvas.id = "bary3dPreview";
  canvas.setAttribute("aria-label", "Interactive 3D Baryonyx preview");
  ...
  status.className = "bary-3d-status";
  status.textContent = "Loading Baryonyx 3D…";
  ...
  async function loadMasks() {
    ...
    ["a", "b", "c", "d", "e"].forEach(pattern => {
      ["male", "female"].forEach(sex => {
        [1, 2].forEach(set => {
          ...
          const source =
            `./assets/textures/baryonyx/${pattern}-${sex}-channels-${set}.png?v=extended-patterns-3`;
          tasks.push(
    ...
    if (failedMasks.length) {
      console.warn(
        `Baryonyx 3D preview continued with ${failedMasks.length} missing mask texture(s).`
      );
    }
  }
  ...
  function installBodyShader(material) {
    ...
    material.customProgramCacheKey = () => "baryonyx-original-layer-masks-v3";
    material.color.set(0xffffff);
    material.needsUpdate = true;
  }

  function isAllosaurus() {
    return dinosaurSelect.value === "bary";
  }

  function showThreeDimensionalError() {
    status.textContent = "Baryonyx 3D preview could not be loaded";
    updateVisibility();
  }
  ...
  try {
    ...
    loader.load(
      ...
      progress => {
        ...
        status.textContent = `Loading Baryonyx 3D · ${percent}%`;
      },
      error => {
        console.error("Baryonyx 3D preview failed:", error);
        failed = true;
