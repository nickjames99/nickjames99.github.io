...
if (stage && dinosaurSelect) {
  // Beipiaosaurus is 3D-only. The shadow root prevents legacy preview code
  // from finding or restyling its WebGL canvas.
  document.getElementById("beipiPreview")?.remove();
  ...
  canvasHost.id = "beipi3dPreviewHost";
  ...
  canvas.id = "beipi3dPreview";
  canvas.setAttribute("aria-label", "Interactive 3D Beipiaosaurus preview");
  ...
  status.className = "beipi-3d-status";
  status.textContent = "Loading Beipiaosaurus 3D…";
  ...
  async function loadMasks() {
    ...
    ["a", "b", "c"].forEach(pattern => {
      ["male", "female"].forEach(sex => {
        [1, 2].forEach(set => {
          ...
          const source =
            `./assets/textures/beipiaosaurus/${pattern}-${sex}-channels-${set}.png`;
          tasks.push(
    ...
    if (failedMasks.length) {
      console.warn(
        `Beipiaosaurus 3D preview continued with ${failedMasks.length} missing mask texture(s).`
      );
    }
  }
  ...
  function installBodyShader(material) {
    ...
    material.customProgramCacheKey = () => "beipiaosaurus-original-layer-masks-v1";
    material.color.set(0xffffff);
    material.needsUpdate = true;
  }

  function isAllosaurus() {
    return dinosaurSelect.value === "beipi";
  }

  function showThreeDimensionalError() {
    status.textContent = "Beipiaosaurus 3D preview could not be loaded";
    updateVisibility();
  }
  ...
  try {
    ...
    loader.load(
      "./assets/models/beipiaosaurus.glb",
      async gltf => {
        await masksReady;
      ...
      progress => {
        ...
        status.textContent = `Loading Beipiaosaurus 3D · ${percent}%`;
      },
      error => {
        console.error("Beipiaosaurus 3D preview failed:", error);
        failed = true;
