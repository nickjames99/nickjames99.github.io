...
if (stage && dinosaurSelect) {
  // Camarasaurus is 3D-only. The shadow root prevents legacy preview code
  // from finding or restyling its WebGL canvas.
  document.getElementById("camaraPreview")?.remove();
  ...
  canvasHost.id = "camara3dPreviewHost";
  ...
  canvas.id = "camara3dPreview";
  canvas.setAttribute("aria-label", "Interactive 3D Camarasaurus preview");
  ...
  status.className = "camara-3d-status";
  status.textContent = "Loading Camarasaurus 3D…";
  ...
  async function loadMasks() {
    ...
    ["a"].forEach(pattern => {
      ["male", "female"].forEach(sex => {
        [1, 2].forEach(set => {
          ...
          const source =
            `./assets/textures/camarasaurus/${pattern}-${sex}-channels-${set}.png`;
          tasks.push(
    ...
    if (failedMasks.length) {
      console.warn(
        `Camarasaurus 3D preview continued with ${failedMasks.length} missing mask texture(s).`
      );
    }
  }
  ...
  function installBodyShader(material) {
    ...
    material.customProgramCacheKey = () => "camarasaurus-original-layer-masks-v1";
    material.color.set(0xffffff);
    material.needsUpdate = true;
  }

  function isAllosaurus() {
    return dinosaurSelect.value === "camara";
  }

  function showThreeDimensionalError() {
    status.textContent = "Camarasaurus 3D preview could not be loaded";
    updateVisibility();
  }
  ...
  try {
    ...
    loader.load(
      "./assets/models/camarasaurus.glb",
      async gltf => {
        await masksReady;
      ...
      progress => {
        ...
        status.textContent = `Loading Camarasaurus 3D · ${percent}%`;
      },
      error => {
        console.error("Camarasaurus 3D preview failed:", error);
        failed = true;
