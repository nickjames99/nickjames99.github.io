...
if (stage && dinosaurSelect) {
  // Carnotaurus is 3D-only. The shadow root prevents legacy preview code
  // from finding or restyling its WebGL canvas.
  document.getElementById("carnoPreview")?.remove();
  ...
  canvasHost.id = "carno3dPreviewHost";
  ...
  canvas.id = "carno3dPreview";
  canvas.setAttribute("aria-label", "Interactive 3D Carnotaurus preview");
  ...
  status.className = "carno-3d-status";
  status.textContent = "Loading Carnotaurus 3D…";
  ...
  async function loadMasks() {
    ...
    ["a", "b", "c", "d"].forEach(pattern => {
      ["male", "female"].forEach(sex => {
        [1, 2].forEach(set => {
          ...
          const source =
            `./assets/textures/carnotaurus/${pattern}-${sex}-channels-${set}.png`;
          tasks.push(
    ...
    if (failedMasks.length) {
      console.warn(
        `Carnotaurus 3D preview continued with ${failedMasks.length} missing mask texture(s).`
      );
    }
  }
  ...
  function installBodyShader(material) {
    ...
    material.customProgramCacheKey = () => "carnotaurus-original-layer-masks-v1";
    material.color.set(0xffffff);
    material.needsUpdate = true;
  }

  function isAllosaurus() {
    return dinosaurSelect.value === "carno";
  }

  function showThreeDimensionalError() {
    status.textContent = "Carnotaurus 3D preview could not be loaded";
    updateVisibility();
  }
  ...
  try {
    ...
    loader.load(
      ...
      progress => {
        ...
        status.textContent = `Loading Carnotaurus 3D · ${percent}%`;
      },
      error => {
        console.error("Carnotaurus 3D preview failed:", error);
        failed = true;
