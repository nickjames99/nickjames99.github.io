...
if (stage && dinosaurSelect) {
  // Diabloceratops is 3D-only. The shadow root prevents legacy preview code
  // from finding or restyling its WebGL canvas.
  document.getElementById("dibblePreview")?.remove();
  ...
  canvasHost.id = "dibble3dPreviewHost";
  ...
  canvas.id = "dibble3dPreview";
  canvas.setAttribute("aria-label", "Interactive 3D Diabloceratops preview");
  ...
  status.className = "dibble-3d-status";
  status.textContent = "Loading Diabloceratops 3D…";
  ...
  async function loadMasks() {
    ...
    ["a", "b", "c"].forEach(pattern => {
      ["male", "female"].forEach(sex => {
        [1, 2].forEach(set => {
          ...
          const source =
            `./assets/textures/diabloceratops/${pattern}-${sex}-channels-${set}.png`;
          tasks.push(
    ...
    if (failedMasks.length) {
      console.warn(
        `Diabloceratops 3D preview continued with ${failedMasks.length} missing mask texture(s).`
      );
    }
  }
  ...
  function installBodyShader(material) {
    ...
    material.customProgramCacheKey = () => "diabloceratops-original-layer-masks-v1";
    material.color.set(0xffffff);
    material.needsUpdate = true;
  }

  function isAllosaurus() {
    return dinosaurSelect.value === "dibble";
  }

  function showThreeDimensionalError() {
    status.textContent = "Diabloceratops 3D preview could not be loaded";
    updateVisibility();
  }
  ...
  try {
    ...
    loader.load(
      "./assets/models/diabloceratops.glb",
      async gltf => {
        await masksReady;
      ...
      progress => {
        ...
        status.textContent = `Loading Diabloceratops 3D · ${percent}%`;
      },
      error => {
        console.error("Diabloceratops 3D preview failed:", error);
        failed = true;
