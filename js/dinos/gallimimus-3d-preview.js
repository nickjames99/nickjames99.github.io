...
if (stage && dinosaurSelect) {
  // Gallimimus is 3D-only. The shadow root prevents legacy preview code
  // from finding or restyling its WebGL canvas.
  document.getElementById("galliPreview")?.remove();
  ...
  canvasHost.id = "galli3dPreviewHost";
  ...
  canvas.id = "galli3dPreview";
  canvas.setAttribute("aria-label", "Interactive 3D Gallimimus preview");
  ...
  status.className = "galli-3d-status";
  status.textContent = "Loading Gallimimus 3D…";
  ...
  async function loadMasks() {
    ...
    ["a", "b", "c"].forEach(pattern => {
      ["male", "female"].forEach(sex => {
        [1, 2].forEach(set => {
          ...
          const source =
            `./assets/textures/gallimimus/${pattern}-${sex}-channels-${set}.png`;
          tasks.push(
    ...
    if (failedMasks.length) {
      console.warn(
        `Gallimimus 3D preview continued with ${failedMasks.length} missing mask texture(s).`
      );
    }
  }
  ...
  function installBodyShader(material) {
    ...
    material.customProgramCacheKey = () => "gallimimus-original-layer-masks-v1";
    material.color.set(0xffffff);
    material.needsUpdate = true;
  }

  function isAllosaurus() {
    return dinosaurSelect.value === "galli";
  }

  function showThreeDimensionalError() {
    status.textContent = "Gallimimus 3D preview could not be loaded";
    updateVisibility();
  }
  ...
  try {
    ...
    loader.load(
      ...
      progress => {
        ...
        status.textContent = `Loading Gallimimus 3D · ${percent}%`;
      },
      error => {
        console.error("Gallimimus 3D preview failed:", error);
        failed = true;
