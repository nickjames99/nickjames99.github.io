...
if (stage && dinosaurSelect) {
  // Herrerasaurus is 3D-only. The shadow root prevents legacy preview code
  // from finding or restyling its WebGL canvas.
  document.getElementById("herraPreview")?.remove();
  ...
  canvasHost.id = "herra3dPreviewHost";
  ...
  canvas.id = "herra3dPreview";
  canvas.setAttribute("aria-label", "Interactive 3D Herrerasaurus preview");
  ...
  status.className = "herra-3d-status";
  status.textContent = "Loading Herrerasaurus 3D…";
  ...
  async function loadMasks() {
    ...
    ["a", "b", "c", "d"].forEach(pattern => {
      ["male", "female"].forEach(sex => {
        [1, 2].forEach(set => {
          ...
          const source =
            `./assets/textures/herrerasaurus/${pattern}-${sex}-channels-${set}.png`;
          tasks.push(
    ...
    if (failedMasks.length) {
      console.warn(
        `Herrerasaurus 3D preview continued with ${failedMasks.length} missing mask texture(s).`
      );
    }
  }
  ...
  function installBodyShader(material) {
    ...
    material.customProgramCacheKey = () => "herrerasaurus-original-layer-masks-v1";
    material.color.set(0xffffff);
    material.needsUpdate = true;
  }

  function isAllosaurus() {
    return dinosaurSelect.value === "herra";
  }

  function showThreeDimensionalError() {
    status.textContent = "Herrerasaurus 3D preview could not be loaded";
    updateVisibility();
  }
  ...
  try {
    ...
    loader.load(
      "./assets/models/herrerasaurus.glb",
      async gltf => {
        await masksReady;
      ...
      progress => {
        ...
        status.textContent = `Loading Herrerasaurus 3D · ${percent}%`;
      },
      error => {
        console.error("Herrerasaurus 3D preview failed:", error);
        failed = true;
