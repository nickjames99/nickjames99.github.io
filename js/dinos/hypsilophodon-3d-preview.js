...
if (stage && dinosaurSelect) {
  // Hypsilophodon is 3D-only. The shadow root prevents legacy preview code
  // from finding or restyling its WebGL canvas.
  document.getElementById("hypsiPreview")?.remove();
  ...
  canvasHost.id = "hypsi3dPreviewHost";
  ...
  canvas.id = "hypsi3dPreview";
  canvas.setAttribute("aria-label", "Interactive 3D Hypsilophodon preview");
  ...
  status.className = "hypsi-3d-status";
  status.textContent = "Loading Hypsilophodon 3D…";
  ...
  async function loadMasks() {
    ...
    ["a", "b", "c"].forEach(pattern => {
      ["male", "female"].forEach(sex => {
        [1, 2].forEach(set => {
          ...
          const source =
            `./assets/textures/hypsilophodon/${pattern}-${sex}-channels-${set}.png`;
          tasks.push(
    ...
    if (failedMasks.length) {
      console.warn(
        `Hypsilophodon 3D preview continued with ${failedMasks.length} missing mask texture(s).`
      );
    }
  }
  ...
  function installBodyShader(material) {
    ...
    material.customProgramCacheKey = () => "hypsilophodon-original-layer-masks-v1";
    material.color.set(0xffffff);
    material.needsUpdate = true;
  }

  function isAllosaurus() {
    return dinosaurSelect.value === "hypsi";
  }

  function showThreeDimensionalError() {
    status.textContent = "Hypsilophodon 3D preview could not be loaded";
    updateVisibility();
  }
  ...
  try {
    ...
    loader.load(
      "./assets/models/hypsilophodon.glb",
      async gltf => {
        await masksReady;
        ...
        gltf.scene.traverse(item => {
          ...
          materials.filter(Boolean).forEach(material => {
            ...
            if (/body|feather/i.test(material.name)) installBodyShader(material);
      ...
      progress => {
        ...
        status.textContent = `Loading Hypsilophodon 3D · ${percent}%`;
      },
      error => {
        console.error("Hypsilophodon 3D preview failed:", error);
        failed = true;
