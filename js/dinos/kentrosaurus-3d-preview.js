...
if (stage && dinosaurSelect) {
  // Kentrosaurus is 3D-only. The shadow root prevents legacy preview code
  // from finding or restyling its WebGL canvas.
  document.getElementById("kentroPreview")?.remove();
  ...
  canvasHost.id = "kentro3dPreviewHost";
  ...
  canvas.id = "kentro3dPreview";
  canvas.setAttribute("aria-label", "Interactive 3D Kentrosaurus preview");
  ...
  status.className = "kentro-3d-status";
  status.textContent = "Loading Kentrosaurus 3D…";
  ...
  async function loadMasks() {
    ...
    ["a", "b", "c"].forEach(pattern => {
      ["male", "female"].forEach(sex => {
        [1, 2].forEach(set => {
          ...
          const source =
            `./assets/textures/kentrosaurus/${pattern}-${sex}-channels-${set}.png`;
          tasks.push(
    ...
    if (failedMasks.length) {
      console.warn(
        `Kentrosaurus 3D preview continued with ${failedMasks.length} missing mask texture(s).`
      );
    }
  }
  ...
  function installBodyShader(material) {
    ...
    material.customProgramCacheKey = () => "kentrosaurus-original-layer-masks-v1";
    material.color.set(0xffffff);
    material.needsUpdate = true;
  }

  function isAllosaurus() {
    return dinosaurSelect.value === "kentro";
  }

  function showThreeDimensionalError() {
    status.textContent = "Kentrosaurus 3D preview could not be loaded";
    updateVisibility();
  }
  ...
  try {
    ...
    loader.load(
      "./assets/models/kentrosaurus.glb",
      async gltf => {
        await masksReady;
      ...
      progress => {
        ...
        status.textContent = `Loading Kentrosaurus 3D · ${percent}%`;
      },
      error => {
        console.error("Kentrosaurus 3D preview failed:", error);
        failed = true;
