...
if (stage && dinosaurSelect) {
  // Maiasaura is 3D-only. The shadow root prevents legacy preview code
  // from finding or restyling its WebGL canvas.
  document.getElementById("maiaPreview")?.remove();
  ...
  canvasHost.id = "maia3dPreviewHost";
  ...
  canvas.id = "maia3dPreview";
  canvas.setAttribute("aria-label", "Interactive 3D Maiasaura preview");
  ...
  status.className = "maia-3d-status";
  status.textContent = "Loading Maiasaura 3D…";
  ...
  async function loadMasks() {
    ...
    ["a", "b", "c"].forEach(pattern => {
      ["male", "female"].forEach(sex => {
        [1, 2].forEach(set => {
          ...
          const source =
            `./assets/textures/maiasaura/${pattern}-${sex}-channels-${set}.png`;
          tasks.push(
    ...
    if (failedMasks.length) {
      console.warn(
        `Maiasaura 3D preview continued with ${failedMasks.length} missing mask texture(s).`
      );
    }
  }
  ...
  function installBodyShader(material) {
    ...
    material.customProgramCacheKey = () => "maiasaura-original-layer-masks-v1";
    material.color.set(0xffffff);
    material.needsUpdate = true;
  }

  function isAllosaurus() {
    return dinosaurSelect.value === "maia";
  }

  function showThreeDimensionalError() {
    status.textContent = "Maiasaura 3D preview could not be loaded";
    updateVisibility();
  }
  ...
  try {
    ...
    loader.load(
      ...
      progress => {
        ...
        status.textContent = `Loading Maiasaura 3D · ${percent}%`;
      },
      error => {
        console.error("Maiasaura 3D preview failed:", error);
        failed = true;
