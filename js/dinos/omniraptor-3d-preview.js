...
if (stage && dinosaurSelect) {
  // Omniraptor is 3D-only. Utahraptor-named source textures belong to this model.
  // from finding or restyling its WebGL canvas.
  document.getElementById("omniPreview")?.remove();
  ...
  canvasHost.id = "omni3dPreviewHost";
  ...
  canvas.id = "omni3dPreview";
  canvas.setAttribute("aria-label", "Interactive 3D Omniraptor preview");
  ...
  status.className = "omni-3d-status";
  status.textContent = "Loading Omniraptor 3D…";
  ...
  async function loadMasks() {
    ...
    ["a", "b", "c", "d", "e"].forEach(pattern => {
      ["male", "female"].forEach(sex => {
        [1, 2].forEach(set => {
          ...
          const source =
            `./assets/textures/omniraptor/${pattern}-${sex}-channels-${set}.png`;
          tasks.push(
    ...
    if (failedMasks.length) {
      console.warn(
        `Omniraptor 3D preview continued with ${failedMasks.length} missing mask texture(s).`
      );
    }
  }
  ...
  function installBodyShader(material) {
    ...
    material.customProgramCacheKey = () => "omniraptor-original-layer-masks-v1";
    material.color.set(0xffffff);
    material.needsUpdate = true;
  }

  function isAllosaurus() {
    return dinosaurSelect.value === "omniraptor";
  }

  function showThreeDimensionalError() {
    status.textContent = "Omniraptor 3D preview could not be loaded";
    updateVisibility();
  }
  ...
  try {
    ...
    loader.load(
      ...
      progress => {
        ...
        status.textContent = `Loading Omniraptor 3D · ${percent}%`;
      },
      error => {
        console.error("Omniraptor 3D preview failed:", error);
        failed = true;
