...
if (stage && dinosaurSelect) {
  // Oviraptor is 3D-only. The shadow root prevents legacy preview code
  // from finding or restyling its WebGL canvas.
  document.getElementById("oviPreview")?.remove();
  ...
  canvasHost.id = "ovi3dPreviewHost";
  ...
  canvas.id = "ovi3dPreview";
  canvas.setAttribute("aria-label", "Interactive 3D Oviraptor preview");
  ...
  status.className = "ovi-3d-status";
  status.textContent = "Loading Oviraptor 3D…";
  ...
  async function loadMasks() {
    ...
    ["a", "b", "c"].forEach(pattern => {
      ["male", "female"].forEach(sex => {
        [1, 2].forEach(set => {
          ...
          const source =
            `./assets/textures/oviraptor/${pattern}-${sex}-channels-${set}.png`;
          tasks.push(
    ...
    if (failedMasks.length) {
      console.warn(
        `Oviraptor 3D preview continued with ${failedMasks.length} missing mask texture(s).`
      );
    }
  }
  ...
  function installBodyShader(material) {
    ...
    material.customProgramCacheKey = () => "oviraptor-original-layer-masks-v1";
    material.color.set(0xffffff);
    material.needsUpdate = true;
  }

  function isAllosaurus() {
    return dinosaurSelect.value === "ovi";
  }

  function showThreeDimensionalError() {
    status.textContent = "Oviraptor 3D preview could not be loaded";
    updateVisibility();
  }
  ...
  try {
    ...
    loader.load(
      ...
      progress => {
        ...
        status.textContent = `Loading Oviraptor 3D · ${percent}%`;
      },
      error => {
        console.error("Oviraptor 3D preview failed:", error);
        failed = true;
