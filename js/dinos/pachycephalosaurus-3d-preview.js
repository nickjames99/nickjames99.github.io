...
if (stage && dinosaurSelect) {
  ...
  canvasHost.id = "pachy3dPreviewHost";
  ...
  canvas.id = "pachy3dPreview";
  canvas.setAttribute("aria-label", "Interactive 3D Pachycephalosaurus preview");
  ...
  status.className = "pachy-3d-status";
  status.textContent = "Loading Pachycephalosaurus 3D…";
  ...
  async function loadMasks() {
    ...
    ["a", "b", "c", "d", "e"].forEach(pattern => {
      ["male", "female"].forEach(sex => {
        [1, 2].forEach(set => {
          ...
          const source =
            `./assets/textures/pachycephalosaurus/${pattern}-${sex}-channels-${set}.png?v=extended-patterns-3`;
          tasks.push(
    ...
    if (failedMasks.length) {
      console.warn(
        `Pachycephalosaurus 3D preview continued with ${failedMasks.length} missing mask texture(s).`
      );
    }
  }
  ...
  function installBodyShader(material) {
    ...
    material.customProgramCacheKey = () => "pachycephalosaurus-original-layer-masks-v3";
    material.color.set(0xffffff);
    material.needsUpdate = true;
  }

  function isAllosaurus() {
    return dinosaurSelect.value === "pachy";
  }

  function showThreeDimensionalError() {
    status.textContent = "Pachycephalosaurus 3D preview could not be loaded";
    updateVisibility();
  }
  ...
  try {
    ...
    loader.load(
      "./assets/models/pachycephalosaurus.glb",
      async gltf => {
        await masksReady;
        ...
        gltf.scene.traverse(item => {
          ...
          materials.filter(Boolean).forEach(material => {
            ...
            if (/body|pachy.*adult/i.test(material.name)) installBodyShader(material);
      ...
      progress => {
        ...
        status.textContent = `Loading Pachycephalosaurus 3D · ${percent}%`;
      },
      error => {
        console.error("Pachycephalosaurus 3D preview failed:", error);
        failed = true;
