...
if (stage && dinosaurSelect) {
  // The 3D model replaces Pteranodon's old fixed top/bottom image views.
  document.getElementById("pteraViewSelect")?.remove();
  document.getElementById("savePreviewBtn")?.remove();

  // Pteranodon is 3D-only. The shadow root prevents legacy preview code
  // from finding or restyling its WebGL canvas.
  document.getElementById("pteraPreview")?.remove();
  ...
  canvasHost.id = "ptera3dPreviewHost";
  ...
  canvas.id = "ptera3dPreview";
  canvas.setAttribute("aria-label", "Interactive 3D Pteranodon preview");
  ...
  status.className = "ptera-3d-status";
  status.textContent = "Loading Pteranodon 3D…";
  ...
  async function loadMasks() {
    ...
    ["a", "b", "c"].forEach(pattern => {
      ["male", "female"].forEach(sex => {
        [1, 2].forEach(set => {
          ...
          const source =
            `./assets/textures/pteranodon/${pattern}-${sex}-channels-${set}.png`;
          tasks.push(
    ...
    if (failedMasks.length) {
      console.warn(
        `Pteranodon 3D preview continued with ${failedMasks.length} missing mask texture(s).`
      );
    }
  }
  ...
  function installBodyShader(material) {
    ...
    material.customProgramCacheKey = () => "pteranodon-original-layer-masks-v1";
    material.color.set(0xffffff);
    material.needsUpdate = true;
  }

  function isAllosaurus() {
    return dinosaurSelect.value === "ptera";
  }

  function showThreeDimensionalError() {
    status.textContent = "Pteranodon 3D preview could not be loaded";
    updateVisibility();
  }
  ...
  try {
    ...
    loader.load(
      ...
      progress => {
        ...
        status.textContent = `Loading Pteranodon 3D · ${percent}%`;
      },
      error => {
        console.error("Pteranodon 3D preview failed:", error);
        failed = true;
