...
if (stage && dinosaurSelect) {
  // Quetzalcoatlus is 3D-only. The shadow root prevents legacy preview code
  // from finding or restyling its WebGL canvas.
  document.getElementById("quetzPreview")?.remove();
  ...
  canvasHost.id = "quetz3dPreviewHost";
  ...
  canvas.id = "quetz3dPreview";
  canvas.setAttribute("aria-label", "Interactive 3D Quetzalcoatlus preview");
  ...
  status.className = "quetz-3d-status";
  status.textContent = "Loading Quetzalcoatlus 3D…";
  ...
  async function loadMasks() {
    ...
    ["a"].forEach(pattern => {
      ["male", "female"].forEach(sex => {
        [1, 2].forEach(set => {
          ...
          const source =
            `./assets/textures/quetzalcoatlus/${pattern}-${sex}-channels-${set}.png`;
          tasks.push(
    ...
    if (failedMasks.length) {
      console.warn(
        `Quetzalcoatlus 3D preview continued with ${failedMasks.length} missing mask texture(s).`
      );
    }
  }
  ...
  function installBodyShader(material) {
    ...
    material.customProgramCacheKey = () => "quetzalcoatlus-original-layer-masks-v1";
    material.color.set(0xffffff);
    material.needsUpdate = true;
  }

  function isAllosaurus() {
    return dinosaurSelect.value === "quetz";
  }

  function showThreeDimensionalError() {
    status.textContent = "Quetzalcoatlus 3D preview could not be loaded";
    updateVisibility();
  }
  ...
  try {
    ...
    loader.load(
      "./assets/models/quetzalcoatlus.glb",
      async gltf => {
        await masksReady;
      ...
      progress => {
        ...
        status.textContent = `Loading Quetzalcoatlus 3D · ${percent}%`;
      },
      error => {
        console.error("Quetzalcoatlus 3D preview failed:", error);
        failed = true;
