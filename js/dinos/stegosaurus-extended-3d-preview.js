...
if (stage && dinosaurSelect) {
  document.getElementById("stego3dPreviewHost")?.remove();
  // Stegosaurus is 3D-only. The shadow root prevents legacy preview code
  // from finding or restyling its WebGL canvas.
  document.getElementById("stegoPreview")?.remove();
  ...
  canvasHost.id = "stego3dPreviewHost";
  ...
  canvas.id = "stego3dPreview";
  canvas.setAttribute("aria-label", "Interactive 3D Stegosaurus preview");
  ...
  status.className = "stego-3d-status";
  status.textContent = "Loading Stegosaurus 3D…";
  ...
  async function loadMasks() {
    ...
    if (failedMasks.length) {
      console.warn(
        `Stegosaurus 3D preview continued with ${failedMasks.length} missing mask texture(s).`
      );
    }
  }
  ...
  function isAllosaurus() {
    return dinosaurSelect.value === "stego";
  }

  function showThreeDimensionalError() {
    status.textContent = "Stegosaurus 3D preview could not be loaded";
    updateVisibility();
  }
  ...
  try {
    ...
    loader.load(
      ...
      progress => {
        ...
        status.textContent = `Loading Stegosaurus 3D · ${percent}%`;
      },
      error => {
        console.error("Stegosaurus 3D preview failed:", error);
        failed = true;
