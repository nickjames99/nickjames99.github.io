...
if (stage && dinosaurSelect) {
  // Tenontosaurus is 3D-only. The shadow root prevents legacy preview code
  // from finding or restyling its WebGL canvas.
  document.getElementById("tenontoPreview")?.remove();
  ...
  canvasHost.id = "tenonto3dPreviewHost";
  ...
  canvas.id = "tenonto3dPreview";
  canvas.setAttribute("aria-label", "Interactive 3D Tenontosaurus preview");
  ...
  status.className = "tenonto-3d-status";
  status.textContent = "Loading Tenontosaurus 3D…";
  ...
  async function loadMasks() {
    ...
    ["a", "b", "c"].forEach(pattern => {
      ["male", "female"].forEach(sex => {
        [1, 2].forEach(set => {
          ...
          const source =
            `./assets/textures/tenontosaurus/${pattern}-${sex}-channels-${set}.png`;
          tasks.push(
    ...
    if (failedMasks.length) {
      console.warn(
        `Tenontosaurus 3D preview continued with ${failedMasks.length} missing mask texture(s).`
      );
    }
  }
  ...
  function installBodyShader(material) {
    ...
    material.customProgramCacheKey = () => "tenontosaurus-original-layer-masks-v1";
    material.color.set(0xffffff);
    material.needsUpdate = true;
  }

  function isAllosaurus() {
    return dinosaurSelect.value === "tenonto";
  }

  function showThreeDimensionalError() {
    status.textContent = "Tenontosaurus 3D preview could not be loaded";
    updateVisibility();
  }
  ...
  try {
    ...
    loader.load(
      "./assets/models/tenontosaurus.glb",
      async gltf => {
        await masksReady;
      ...
      progress => {
        ...
        status.textContent = `Loading Tenontosaurus 3D · ${percent}%`;
      },
      error => {
        console.error("Tenontosaurus 3D preview failed:", error);
        failed = true;
