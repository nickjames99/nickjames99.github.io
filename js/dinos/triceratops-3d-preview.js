...
if (stage && dinosaurSelect) {
  // Triceratops is 3D-only. The shadow root prevents legacy preview code
  // from finding or restyling its WebGL canvas.
  document.getElementById("trikePreview")?.remove();
  ...
  canvasHost.id = "trike3dPreviewHost";
  ...
  canvas.id = "trike3dPreview";
  canvas.setAttribute("aria-label", "Interactive 3D Triceratops preview");
  ...
  status.className = "trike-3d-status";
  status.textContent = "Loading Triceratops 3D…";
  ...
  async function loadMasks() {
    ...
    const tasks = [];
    ["a", "b", "c", "d", "e", "f"].forEach(pattern => {
      ["male", "female"].forEach(sex => {
        [1, 2].forEach(set => {
          const key = `${pattern}-${sex}-${set}`;
          const source =
            `./assets/textures/triceratops/${pattern}-${sex}-channels-${set}.png`;
          tasks.push(
    ...
    if (failedMasks.length) {
      console.warn(
        `Triceratops 3D preview continued with ${failedMasks.length} missing mask texture(s).`
      );
    }
  }
  ...
  function activeMasks() {
    ...
    const selectedPattern = override?.species === dinosaurSelect?.value
      ? override.pattern
      : patternSelect?.value;
    const pattern = ["A", "B", "C", "D", "E", "F"].includes(selectedPattern)
  ...
  function installBodyShader(material) {
    ...
    material.customProgramCacheKey = () => "triceratops-original-layer-masks-v2";
    material.color.set(0xffffff);
    material.needsUpdate = true;
  }

  function isAllosaurus() {
    return dinosaurSelect.value === "trike";
  }

  function showThreeDimensionalError() {
    status.textContent = "Triceratops 3D preview could not be loaded";
    updateVisibility();
  }
  ...
  try {
    ...
    loader.load(
      ...
      progress => {
        ...
        status.textContent = `Loading Triceratops 3D · ${percent}%`;
      },
      error => {
        console.error("Triceratops 3D preview failed:", error);
        failed = true;
