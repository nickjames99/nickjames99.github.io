...
if (stage && dinosaurSelect) {
  // Troodon is 3D-only. The shadow root prevents legacy preview code
  // from finding or restyling its WebGL canvas.
  document.getElementById("troodonPreview")?.remove();
  ...
  canvasHost.id = "troodon3dPreviewHost";
  ...
  canvas.id = "troodon3dPreview";
  canvas.setAttribute("aria-label", "Interactive 3D Troodon preview");
  ...
  status.className = "troodon-3d-status";
  status.textContent = "Loading Troodon 3D…";
  ...
  async function loadMasks() {
    ...
    ["a", "b", "c"].forEach(pattern => {
      ["male", "female"].forEach(sex => {
        [1, 2].forEach(set => {
          ...
          const source =
            `./assets/textures/troodon/${pattern}-${sex}-channels-${set}.png`;
          tasks.push(
    ...
    if (failedMasks.length) {
      console.warn(
        `Troodon 3D preview continued with ${failedMasks.length} missing mask texture(s).`
      );
    }
  }
  ...
  function installBodyShader(material) {
    ...
    material.customProgramCacheKey = () => "troodon-original-layer-masks-v1";
    material.color.set(0xffffff);
    material.needsUpdate = true;
  }

  function isAllosaurus() {
    return dinosaurSelect.value === "troodon";
  }

  function showThreeDimensionalError() {
    status.textContent = "Troodon 3D preview could not be loaded";
    updateVisibility();
  }
  ...
  try {
    ...
    loader.load(
      ...
      progress => {
        ...
        status.textContent = `Loading Troodon 3D · ${percent}%`;
      },
      error => {
        console.error("Troodon 3D preview failed:", error);
        failed = true;
