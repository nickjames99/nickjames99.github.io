...
if (stage && dinosaurSelect) {
  // Tyrannosaurus is 3D-only. The shadow root prevents legacy preview code
  // from finding or restyling its WebGL canvas.
  document.getElementById("trexPreview")?.remove();
  ...
  canvasHost.id = "trex3dPreviewHost";
  ...
  canvas.id = "trex3dPreview";
  canvas.setAttribute("aria-label", "Interactive 3D Tyrannosaurus preview");
  ...
  status.className = "trex-3d-status";
  status.textContent = "Loading Tyrannosaurus 3D…";
  ...
  async function loadMasks() {
    ...
    ["a", "b", "c", "d"].forEach(pattern => {
      ["male", "female"].forEach(sex => {
        [1, 2].forEach(set => {
          ...
          const source =
            `./assets/textures/tyrannosaurus/${pattern}-${sex}-channels-${set}.png`;
          tasks.push(
    ...
    if (failedMasks.length) {
      console.warn(
        `Tyrannosaurus 3D preview continued with ${failedMasks.length} missing mask texture(s).`
      );
    }
  }
  ...
  function isAllosaurus() {
    return dinosaurSelect.value === "trex";
  }

  function showThreeDimensionalError() {
    status.textContent = "Tyrannosaurus 3D preview could not be loaded";
    updateVisibility();
  }
  ...
  try {
    ...
    loader.load(
      "./assets/models/tyrannosaurus.glb",
      async gltf => {
        await masksReady;
      ...
      progress => {
        ...
        status.textContent = `Loading Tyrannosaurus 3D · ${percent}%`;
      },
      error => {
        console.error("Tyrannosaurus 3D preview failed:", error);
        failed = true;
