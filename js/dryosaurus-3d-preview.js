...
if (stage && dinosaurSelect) {
  // Dryosaurus is 3D-only. The shadow root prevents legacy preview code
  // from finding or restyling its WebGL canvas.
  document.getElementById("dryoPreview")?.remove();
  ...
  canvasHost.id = "dryo3dPreviewHost";
  ...
  canvas.id = "dryo3dPreview";
  canvas.setAttribute("aria-label", "Interactive 3D Dryosaurus preview");
  ...
  status.className = "dryo-3d-status";
  status.textContent = "Loading Dryosaurus 3D…";
  ...
  async function loadMasks() {
    ...
    ["a", "b", "c"].forEach(pattern => {
      ["male", "female"].forEach(sex => {
        [1, 2].forEach(set => {
          ...
          const source =
            `./assets/textures/dryosaurus/${pattern}-${sex}-channels-${set}.png`;
          tasks.push(
    ...
    if (failedMasks.length) {
      console.warn(
        `Dryosaurus 3D preview continued with ${failedMasks.length} missing mask texture(s).`
      );
    }
  }
  ...
  function installBodyShader(material) {
    ...
    material.customProgramCacheKey = () => "dryosaurus-original-layer-masks-v1";
    material.color.set(0xffffff);
    material.needsUpdate = true;
  }

  function isAllosaurus() {
    return dinosaurSelect.value === "dryo";
  }

  function showThreeDimensionalError() {
    status.textContent = "Dryosaurus 3D preview could not be loaded";
    updateVisibility();
  }
  ...
  try {
    ...
    loader.load(
      ...
      progress => {
        ...
        status.textContent = `Loading Dryosaurus 3D · ${percent}%`;
      },
      error => {
        console.error("Dryosaurus 3D preview failed:", error);
        failed = true;
