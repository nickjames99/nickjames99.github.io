(() => {
  const SOURCES = Object.fromEntries(
    ["A", "B", "C"].map(pattern => {
      ...
      return [pattern, {
        base: `./assets/embedded/allosaurus/${folder}/base.png`,
        body: `./assets/embedded/allosaurus/${folder}/body.png`,
        detail: `./assets/embedded/allosaurus/${folder}/detail.png`,
        dominant: `./assets/embedded/allosaurus/${folder}/dominant.png`,
        eyes: `./assets/embedded/allosaurus/${folder}/eyes.png`,
        flank: `./assets/embedded/allosaurus/${folder}/flank.png`,
        markings: `./assets/embedded/allosaurus/${folder}/markings.png`,
        underside: `./assets/embedded/allosaurus/${folder}/underside.png`
      }];
    })
  );
  const CHANNELS = ["dominant", "markings", "flank", "detail", "body", "underside", "eyes"];

  const canvas = document.getElementById("alloPreview");
  ...
  async function ensureLoaded(pattern) {
    ...
    const loading = (async () => {
      ...
      state.loading.delete(pattern);
      console.error(`Allosaurus Pattern ${pattern} failed to load:`, error);
      return null;
    });
  ...
  function updateControls() {
    const active = dinosaurSelect.value === "allo";
    if (!active) {
    ...
    if (speciesSelect) speciesSelect.value = "0";
    if (badge) badge.textContent =
      `Allosaurus · Pattern ${patternSelect.value}`;
  ...
  patternSelect.addEventListener("change", () => {
    if (dinosaurSelect.value === "allo") scheduleRender(0);
  });
  sexSelect?.addEventListener("change", () => {
    if (dinosaurSelect.value === "allo") scheduleRender(0);
  });
  pickerContainer?.addEventListener("input", () => {
    if (dinosaurSelect.value === "allo") scheduleRender();
  });
  ...
  saveButton?.addEventListener("click", event => {
    if (dinosaurSelect.value !== "allo") return;
    event.preventDefault();
    event.stopImmediatePropagation();
    const link = document.createElement("a");
    link.download = `allosaurus-pattern-${patternSelect.value.toLowerCase()}.png`;
