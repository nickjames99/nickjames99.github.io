(() => {
  const SOURCES = Object.fromEntries(
    ["A", "B", "C"].map(pattern => {
      ...
      return [pattern, {
        base: `./assets/embedded/beipiaosaurus/${folder}/base.png`,
        body: `./assets/embedded/beipiaosaurus/${folder}/body.png`,
        detail: `./assets/embedded/beipiaosaurus/${folder}/detail.png`,
        dominant: `./assets/embedded/beipiaosaurus/${folder}/dominant.png`,
        eyes: `./assets/embedded/beipiaosaurus/${folder}/eyes.png`,
        flank: `./assets/embedded/beipiaosaurus/${folder}/flank.png`,
        markings: `./assets/embedded/beipiaosaurus/${folder}/markings.png`,
        underside: `./assets/embedded/beipiaosaurus/${folder}/underside.png`
      }];
    })
  );
  const CHANNELS = ["dominant", "markings", "flank", "detail", "body", "underside", "eyes"];

  const canvas = document.getElementById("beipiPreview");
  ...
  async function ensureLoaded(pattern) {
    ...
    const loading = (async () => {
      ...
      state.loading.delete(pattern);
      console.error(`Beipiaosaurus Pattern ${pattern} failed to load:`, error);
      return null;
    });
  ...
  function updateControls() {
    const active = dinosaurSelect.value === "beipi";
    if (!active) {
    ...
    if (speciesSelect) speciesSelect.value = "1";
    if (badge) badge.textContent =
      `Beipiaosaurus · Pattern ${patternSelect.value}`;
  ...
  patternSelect.addEventListener("change", () => {
    if (dinosaurSelect.value === "beipi") scheduleRender(0);
  });
  sexSelect?.addEventListener("change", () => {
    if (dinosaurSelect.value === "beipi") scheduleRender(0);
  });
  pickerContainer?.addEventListener("input", () => {
    if (dinosaurSelect.value === "beipi") scheduleRender();
  });
  ...
  saveButton?.addEventListener("click", event => {
    if (dinosaurSelect.value !== "beipi") return;
    event.preventDefault();
    event.stopImmediatePropagation();
    const link = document.createElement("a");
    link.download = `beipiaosaurus-pattern-${patternSelect.value.toLowerCase()}.png`;
