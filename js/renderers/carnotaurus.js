(() => {
  const SOURCES = Object.fromEntries(
    ["A", "B", "C", "D"].map(pattern => {
      ...
      return [pattern, {
        base: `./assets/embedded/carnotaurus/${folder}/base.png`,
        body: `./assets/embedded/carnotaurus/${folder}/body.png`,
        detail: `./assets/embedded/carnotaurus/${folder}/detail.png`,
        dominant: `./assets/embedded/carnotaurus/${folder}/dominant.png`,
        eyes: `./assets/embedded/carnotaurus/${folder}/eyes.png`,
        flank: `./assets/embedded/carnotaurus/${folder}/flank.png`,
        markings: `./assets/embedded/carnotaurus/${folder}/markings.png`,
        underside: `./assets/embedded/carnotaurus/${folder}/underside.png`
      }];
    })
  );
  const CHANNELS = ["dominant", "markings", "flank", "detail", "body", "underside", "eyes"];

  const canvas = document.getElementById("carnoPreview");
  ...
  async function ensureLoaded(pattern) {
    ...
    const loading = (async () => {
      ...
      state.loading.delete(pattern);
      console.error(`Carnotaurus Pattern ${pattern} failed to load:`, error);
      return null;
    });
  ...
  function updateControls() {
    const active = dinosaurSelect.value === "carno";
    if (!active) {
    ...
    if (speciesSelect) speciesSelect.value = "2";
    if (badge) badge.textContent =
      `Carnotaurus · Pattern ${patternSelect.value}`;
  ...
  patternSelect.addEventListener("change", () => {
    if (dinosaurSelect.value === "carno") scheduleRender(0);
  });
  sexSelect?.addEventListener("change", () => {
    if (dinosaurSelect.value === "carno") scheduleRender(0);
  });
  pickerContainer?.addEventListener("input", () => {
    if (dinosaurSelect.value === "carno") scheduleRender();
  });
  ...
  saveButton?.addEventListener("click", event => {
    if (dinosaurSelect.value !== "carno") return;
    event.preventDefault();
    event.stopImmediatePropagation();
    const link = document.createElement("a");
    link.download = `carnotaurus-pattern-${patternSelect.value.toLowerCase()}.png`;
