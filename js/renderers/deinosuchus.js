(() => {
  const SOURCES = Object.fromEntries(
    ["A", "B", "C"].map(pattern => {
      ...
      return [pattern, {
        base: `./assets/embedded/deinosuchus/${folder}/base.png`,
        body: `./assets/embedded/deinosuchus/${folder}/body.png`,
        detail: `./assets/embedded/deinosuchus/${folder}/detail.png`,
        dominant: `./assets/embedded/deinosuchus/${folder}/dominant.png`,
        eyes: `./assets/embedded/deinosuchus/${folder}/eyes.png`,
        flank: `./assets/embedded/deinosuchus/${folder}/flank.png`,
        markings: `./assets/embedded/deinosuchus/${folder}/markings.png`,
        underside: `./assets/embedded/deinosuchus/${folder}/underside.png`
      }];
    })
  );
  const CHANNELS = ["dominant", "markings", "flank", "detail", "body", "underside", "eyes"];

  const canvas = document.getElementById("deinoPreview");
  ...
  async function ensureLoaded(pattern) {
    ...
    const loading = (async () => {
      ...
      state.loading.delete(pattern);
      console.error(`Deinosuchus Pattern ${pattern} failed to load:`, error);
      return null;
    });
  ...
  function updateControls() {
    const active = dinosaurSelect.value === "deino";
    if (!active) {
    ...
    if (badge) badge.textContent =
      `Deinosuchus · Pattern ${patternSelect.value}`;
  ...
  patternSelect.addEventListener("change", () => {
    if (dinosaurSelect.value === "deino") scheduleRender(0);
  });
  sexSelect?.addEventListener("change", () => {
    if (dinosaurSelect.value === "deino") scheduleRender(0);
  });
  pickerContainer?.addEventListener("input", () => {
    if (dinosaurSelect.value === "deino") scheduleRender();
  });
  ...
  saveButton?.addEventListener("click", event => {
    if (dinosaurSelect.value !== "deino") return;
    event.preventDefault();
    event.stopImmediatePropagation();
    const link = document.createElement("a");
    link.download = `deinosuchus-pattern-${patternSelect.value.toLowerCase()}.png`;
