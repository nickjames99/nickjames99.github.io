(() => {
  const SOURCES = Object.fromEntries(
    ["A", "B", "C"].map(pattern => {
      ...
      return [pattern, {
        base: `./assets/embedded/diabloceratops/${folder}/base.png`,
        body: `./assets/embedded/diabloceratops/${folder}/body.png`,
        detail: `./assets/embedded/diabloceratops/${folder}/detail.png`,
        dominant: `./assets/embedded/diabloceratops/${folder}/dominant.png`,
        eyes: `./assets/embedded/diabloceratops/${folder}/eyes.png`,
        flank: `./assets/embedded/diabloceratops/${folder}/flank.png`,
        markings: `./assets/embedded/diabloceratops/${folder}/markings.png`,
        underside: `./assets/embedded/diabloceratops/${folder}/underside.png`
      }];
    })
  );
  const CHANNELS = ["dominant", "markings", "flank", "detail", "body", "underside", "eyes"];

  const canvas = document.getElementById("dibblePreview");
  ...
  async function ensureLoaded(pattern) {
    ...
    const loading = (async () => {
      ...
      state.loading.delete(pattern);
      console.error(`Diabloceratops Pattern ${pattern} failed to load:`, error);
      return null;
    });
  ...
  function updateControls() {
    const active = dinosaurSelect.value === "dibble";
    if (!active) {
    ...
    if (speciesSelect) speciesSelect.value = "5";
    if (badge) badge.textContent =
      `Diabloceratops · Pattern ${patternSelect.value}`;
  ...
  dinosaurSelect.addEventListener("change", () => {
    if (dinosaurSelect.value === "dibble") {
      updateControls();
      scheduleRender(0);
    } else {
      canvas.style.setProperty("display", "none", "important");
  ...
  patternSelect.addEventListener("change", () => {
    if (dinosaurSelect.value === "dibble") scheduleRender(0);
  });
  sexSelect?.addEventListener("change", () => {
    if (dinosaurSelect.value === "dibble") scheduleRender(0);
  });
  pickerContainer?.addEventListener("input", () => {
    if (dinosaurSelect.value === "dibble") scheduleRender();
  });
  ...
  saveButton?.addEventListener("click", event => {
    if (dinosaurSelect.value !== "dibble") return;
    event.preventDefault();
    event.stopImmediatePropagation();
    const link = document.createElement("a");
    link.download = `diabloceratops-pattern-${patternSelect.value.toLowerCase()}.png`;
