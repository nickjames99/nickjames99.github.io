(() => {
  const SOURCES = Object.fromEntries(
    ["A", "B", "C"].map(pattern => {
      ...
      return [pattern, {
        base: `./assets/embedded/dilophosaurus/${folder}/base.png`,
        body: `./assets/embedded/dilophosaurus/${folder}/body.png`,
        detail: `./assets/embedded/dilophosaurus/${folder}/detail.png`,
        dominant: `./assets/embedded/dilophosaurus/${folder}/dominant.png`,
        eyes: `./assets/embedded/dilophosaurus/${folder}/eyes.png`,
        flank: `./assets/embedded/dilophosaurus/${folder}/flank.png`,
        markings: `./assets/embedded/dilophosaurus/${folder}/markings.png`,
        underside: `./assets/embedded/dilophosaurus/${folder}/underside.png`
      }];
    })
  );
  const CHANNELS = ["dominant", "markings", "flank", "detail", "body", "underside", "eyes"];

  const canvas = document.getElementById("diloPreview");
  ...
  async function ensureLoaded(pattern) {
    ...
    const loading = (async () => {
      ...
      state.loading.delete(pattern);
      console.error(`Dilophosaurus Pattern ${pattern} failed to load:`, error);
      return null;
    });
  ...
  function updateControls() {
    const active = dinosaurSelect.value === "dilo";
    if (!active) {
    ...
    if (speciesSelect) speciesSelect.value = "6";
    if (badge) badge.textContent =
      `Dilophosaurus · Pattern ${patternSelect.value}`;
  ...
  patternSelect.addEventListener("change", () => {
    if (dinosaurSelect.value === "dilo") scheduleRender(0);
  });
  sexSelect?.addEventListener("change", () => {
    if (dinosaurSelect.value === "dilo") scheduleRender(0);
  });
  pickerContainer?.addEventListener("input", () => {
    if (dinosaurSelect.value === "dilo") scheduleRender();
  });
  ...
  saveButton?.addEventListener("click", event => {
    if (dinosaurSelect.value !== "dilo") return;
    event.preventDefault();
    event.stopImmediatePropagation();
    const link = document.createElement("a");
    link.download = `dilophosaurus-pattern-${patternSelect.value.toLowerCase()}.png`;
