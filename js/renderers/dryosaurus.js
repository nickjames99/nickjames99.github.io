(() => {
  const SOURCES = Object.fromEntries(
    ["A", "B", "C"].map(pattern => {
      ...
      return [pattern, {
        base: `./assets/embedded/dryosaurus/${folder}/base.png`,
        body: `./assets/embedded/dryosaurus/${folder}/body.png`,
        detail: `./assets/embedded/dryosaurus/${folder}/detail.png`,
        dominant: `./assets/embedded/dryosaurus/${folder}/dominant.png`,
        eyes: `./assets/embedded/dryosaurus/${folder}/eyes.png`,
        flank: `./assets/embedded/dryosaurus/${folder}/flank.png`,
        markings: `./assets/embedded/dryosaurus/${folder}/markings.png`,
        underside: `./assets/embedded/dryosaurus/${folder}/underside.png`
      }];
    })
  );
  const CHANNELS = ["dominant", "markings", "flank", "detail", "body", "underside", "eyes"];

  const canvas = document.getElementById("dryoPreview");
  ...
  async function ensureLoaded(pattern) {
    ...
    const loading = (async () => {
      ...
      state.loading.delete(pattern);
      console.error(`Dryosaurus Pattern ${pattern} failed to load:`, error);
      return null;
    });
  ...
  function updateControls() {
    const active = dinosaurSelect.value === "dryo";
    if (!active) {
    ...
    if (speciesSelect) speciesSelect.value = "7";
    if (badge) badge.textContent =
      `Dryosaurus · Pattern ${patternSelect.value}`;
  ...
  patternSelect.addEventListener("change", () => {
    if (dinosaurSelect.value === "dryo") scheduleRender(0);
  });
  sexSelect?.addEventListener("change", () => {
    if (dinosaurSelect.value === "dryo") scheduleRender(0);
  });
  pickerContainer?.addEventListener("input", () => {
    if (dinosaurSelect.value === "dryo") scheduleRender();
  });
  ...
  saveButton?.addEventListener("click", event => {
    if (dinosaurSelect.value !== "dryo") return;
    event.preventDefault();
    event.stopImmediatePropagation();
    const link = document.createElement("a");
    link.download = `dryosaurus-pattern-${patternSelect.value.toLowerCase()}.png`;
