(() => {
  const SOURCES = Object.fromEntries(
    ["A", "B", "C"].map(pattern => {
      ...
      return [pattern, {
        base: `./assets/embedded/gallimimus/${folder}/base.png`,
        body: `./assets/embedded/gallimimus/${folder}/body.png`,
        detail: `./assets/embedded/gallimimus/${folder}/detail.png`,
        dominant: `./assets/embedded/gallimimus/${folder}/dominant.png`,
        eyes: `./assets/embedded/gallimimus/${folder}/eyes.png`,
        flank: `./assets/embedded/gallimimus/${folder}/flank.png`,
        markings: `./assets/embedded/gallimimus/${folder}/markings.png`,
        underside: `./assets/embedded/gallimimus/${folder}/underside.png`
      }];
    })
  );
  const CHANNELS = ["dominant", "markings", "flank", "detail", "body", "underside", "eyes"];

  const canvas = document.getElementById("galliPreview");
  ...
  async function ensureLoaded(pattern) {
    ...
    const loading = (async () => {
      ...
      state.loading.delete(pattern);
      console.error(`Gallimimus Pattern ${pattern} failed to load:`, error);
      return null;
    });
  ...
  function updateControls() {
    const active = dinosaurSelect.value === "galli";
    if (!active) {
    ...
    if (speciesSelect) speciesSelect.value = "8";
    if (badge) badge.textContent =
      `Gallimimus · Pattern ${patternSelect.value}`;
  ...
  patternSelect.addEventListener("change", () => {
    if (dinosaurSelect.value === "galli") scheduleRender(0);
  });
  sexSelect?.addEventListener("change", () => {
    if (dinosaurSelect.value === "galli") scheduleRender(0);
  });
  pickerContainer?.addEventListener("input", () => {
    if (dinosaurSelect.value === "galli") scheduleRender();
  });
  ...
  saveButton?.addEventListener("click", event => {
    if (dinosaurSelect.value !== "galli") return;
    event.preventDefault();
    event.stopImmediatePropagation();
    const link = document.createElement("a");
    link.download = `gallimimus-pattern-${patternSelect.value.toLowerCase()}.png`;
