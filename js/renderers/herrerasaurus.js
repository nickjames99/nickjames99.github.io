(() => {
  const SOURCES = Object.fromEntries(
    ["A", "B", "C", "D"].map(pattern => {
      ...
      return [pattern, {
        base: `./assets/embedded/herrerasaurus/${folder}/base.png`,
        body: `./assets/embedded/herrerasaurus/${folder}/body.png`,
        detail: `./assets/embedded/herrerasaurus/${folder}/detail.png`,
        dominant: `./assets/embedded/herrerasaurus/${folder}/dominant.png`,
        eyes: `./assets/embedded/herrerasaurus/${folder}/eyes.png`,
        flank: `./assets/embedded/herrerasaurus/${folder}/flank.png`,
        markings: `./assets/embedded/herrerasaurus/${folder}/markings.png`,
        underside: `./assets/embedded/herrerasaurus/${folder}/underside.png`
      }];
    })
  );
  const CHANNELS = ["dominant", "markings", "flank", "detail", "body", "underside", "eyes"];

  const canvas = document.getElementById("herraPreview");
  ...
  async function ensureLoaded(pattern) {
    ...
    const loading = (async () => {
      ...
      state.loading.delete(pattern);
      console.error(`Herrerasaurus Pattern ${pattern} failed to load:`, error);
      return null;
    });
  ...
  function updateControls() {
    const active = dinosaurSelect.value === "herra";
    if (!active) {
    ...
    if (speciesSelect) speciesSelect.value = "9";
    if (badge) badge.textContent =
      `Herrerasaurus · Pattern ${patternSelect.value}`;
  ...
  patternSelect.addEventListener("change", () => {
    if (dinosaurSelect.value === "herra") scheduleRender(0);
  });
  sexSelect?.addEventListener("change", () => {
    if (dinosaurSelect.value === "herra") scheduleRender(0);
  });
  pickerContainer?.addEventListener("input", () => {
    if (dinosaurSelect.value === "herra") scheduleRender();
  });
  ...
  saveButton?.addEventListener("click", event => {
    if (dinosaurSelect.value !== "herra") return;
    event.preventDefault();
    event.stopImmediatePropagation();
    const link = document.createElement("a");
    link.download = `herrerasaurus-pattern-${patternSelect.value.toLowerCase()}.png`;
