(() => {
  ...
  function updateControls() {
    const active = dinosaurSelect.value === "hypsi";
    if (!active) {
  ...
  patternSelect.addEventListener("change", () => {
    if (dinosaurSelect.value === "hypsi") scheduleRender(0);
  });
  sexSelect?.addEventListener("change", () => {
    if (dinosaurSelect.value === "hypsi") scheduleRender(0);
  });
  pickerContainer?.addEventListener("input", () => {
    if (dinosaurSelect.value === "hypsi") scheduleRender();
  });
  ...
  saveButton?.addEventListener("click", event => {
    if (dinosaurSelect.value !== "hypsi") return;
