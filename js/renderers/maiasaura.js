(() => {
  const SOURCES = Object.fromEntries(
    ["A", "B", "C"].map(pattern => {
      ...
      return [pattern, {
        base: `./assets/embedded/maiasaura/${folder}/base.png`,
        body: `./assets/embedded/maiasaura/${folder}/body.png`,
        detail: `./assets/embedded/maiasaura/${folder}/detail.png`,
        dominant: `./assets/embedded/maiasaura/${folder}/dominant.png`,
        eyes: `./assets/embedded/maiasaura/${folder}/eyes.png`,
        flank: `./assets/embedded/maiasaura/${folder}/flank.png`,
        markings: `./assets/embedded/maiasaura/${folder}/markings.png`,
        underside: `./assets/embedded/maiasaura/${folder}/underside.png`
      }];
    })
  );
  const CHANNELS = ["dominant", "markings", "flank", "detail", "body", "underside", "eyes"];

  const canvas = document.getElementById("maiaPreview");
  ...
  async function ensureLoaded(pattern) {
    ...
    const loading = (async () => {
      ...
      state.loading.delete(pattern);
      console.error(`Maiasaura Pattern ${pattern} failed to load:`, error);
      return null;
    });
  ...
  function updateControls() {
    const active = dinosaurSelect.value === "maia";
    if (!active) {
    ...
    if (speciesSelect) speciesSelect.value = "11";
    if (badge) badge.textContent =
      `Maiasaura · Pattern ${patternSelect.value}`;
  ...
  patternSelect.addEventListener("change", () => {
    if (dinosaurSelect.value === "maia") scheduleRender(0);
  });
  sexSelect?.addEventListener("change", () => {
    if (dinosaurSelect.value === "maia") scheduleRender(0);
  });
  pickerContainer?.addEventListener("input", () => {
    if (dinosaurSelect.value === "maia") scheduleRender();
  });
  ...
  saveButton?.addEventListener("click", event => {
    if (dinosaurSelect.value !== "maia") return;
    event.preventDefault();
    event.stopImmediatePropagation();
    const link = document.createElement("a");
    link.download = `maiasaura-pattern-${patternSelect.value.toLowerCase()}.png`;
