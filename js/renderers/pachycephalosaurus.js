(() => {
  const SOURCES = Object.fromEntries(
    ["A", "B", "C", "D"].map(pattern => {
      ...
      return [pattern, {
        base: `./assets/embedded/pachycephalosaurus/${folder}/base.png`,
        body: `./assets/embedded/pachycephalosaurus/${folder}/body.png`,
        detail: `./assets/embedded/pachycephalosaurus/${folder}/detail.png`,
        dominant: `./assets/embedded/pachycephalosaurus/${folder}/dominant.png`,
        eyes: `./assets/embedded/pachycephalosaurus/${folder}/eyes.png`,
        flank: `./assets/embedded/pachycephalosaurus/${folder}/flank.png`,
        markings: `./assets/embedded/pachycephalosaurus/${folder}/markings.png`,
        underside: `./assets/embedded/pachycephalosaurus/${folder}/underside.png`
      }];
    })
  );
  const CHANNELS = ["dominant", "markings", "flank", "detail", "body", "underside", "eyes"];

  const canvas = document.getElementById("pachyPreview");
  ...
  async function ensureLoaded(pattern) {
    ...
    const loading = (async () => {
      ...
      state.loading.delete(pattern);
      console.error(`Pachycephalosaurus Pattern ${pattern} failed to load:`, error);
      return null;
    });
  ...
  function updateControls() {
    const active = dinosaurSelect.value === "pachy";
    if (!active) {
    ...
    [...patternSelect.options].forEach(option => {
      const available = ["A", "B", "C", "D"].includes(option.value);
      option.disabled = !available;
      option.hidden = !available;
    });
    if (!["A", "B", "C", "D"].includes(patternSelect.value)) {
      patternSelect.value = "A";
    }
    patternSelect.disabled = false;
    if (speciesSelect) speciesSelect.value = "13";
    if (badge) badge.textContent =
      `Pachycephalosaurus · Pattern ${patternSelect.value}`;
  ...
  patternSelect.addEventListener("change", () => {
    if (dinosaurSelect.value === "pachy") scheduleRender(0);
  });
  sexSelect?.addEventListener("change", () => {
    if (dinosaurSelect.value === "pachy") scheduleRender(0);
  });
  pickerContainer?.addEventListener("input", () => {
    if (dinosaurSelect.value === "pachy") scheduleRender();
  });
  ...
  saveButton?.addEventListener("click", event => {
    if (dinosaurSelect.value !== "pachy") return;
    event.preventDefault();
    event.stopImmediatePropagation();
    const link = document.createElement("a");
    link.download = `pachycephalosaurus-pattern-${patternSelect.value.toLowerCase()}.png`;
