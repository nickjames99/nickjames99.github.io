(() => {
  const SOURCES = Object.fromEntries(
    ["A", "B", "C"].map(pattern => {
      ...
      return [pattern, {
        base: `./assets/embedded/triceratops/${folder}/base.png`,
        body: `./assets/embedded/triceratops/${folder}/body.png`,
        detail: `./assets/embedded/triceratops/${folder}/detail.png`,
        dominant: `./assets/embedded/triceratops/${folder}/dominant.png`,
        eyes: `./assets/embedded/triceratops/${folder}/eyes.png`,
        flank: `./assets/embedded/triceratops/${folder}/flank.png`,
        markings: `./assets/embedded/triceratops/${folder}/markings.png`,
        underside: `./assets/embedded/triceratops/${folder}/underside.png`
      }];
    })
  );
  const CHANNELS = ["dominant", "markings", "flank", "detail", "body", "underside", "eyes"];

  const canvas = document.getElementById("trikePreview");
  ...
  async function ensureLoaded(pattern) {
    ...
    const loading = (async () => {
      ...
      state.loading.delete(pattern);
      console.error(`Triceratops Pattern ${pattern} failed to load:`, error);
      return null;
    });
  ...
  function updateControls() {
    const active = dinosaurSelect.value === "trike";
    if (!active) {
    ...
    if (speciesSelect) speciesSelect.value = "17";
    if (badge) badge.textContent =
      `Triceratops · Pattern ${patternSelect.value}`;
  ...
  patternSelect.addEventListener("change", () => {
    if (dinosaurSelect.value === "trike") scheduleRender(0);
  });
  sexSelect?.addEventListener("change", () => {
    if (dinosaurSelect.value === "trike") scheduleRender(0);
  });
  pickerContainer?.addEventListener("input", () => {
    if (dinosaurSelect.value === "trike") scheduleRender();
  });
  ...
  saveButton?.addEventListener("click", event => {
    if (dinosaurSelect.value !== "trike") return;
    event.preventDefault();
    event.stopImmediatePropagation();
    const link = document.createElement("a");
    link.download = `triceratops-pattern-${patternSelect.value.toLowerCase()}.png`;
